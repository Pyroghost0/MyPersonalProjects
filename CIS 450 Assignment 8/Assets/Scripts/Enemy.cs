...
#pragma warning restore CS0108 // Member hides inherited member; missing new keyword
        ...
        if (!knockback)
        {
            if (target == null)
            {
                ...
                if (enemies.Length > 0)
                {
                    ...
                    {
                        if ((enemies[i].transform.position - transform.position).magnitude < (closest.position - transform.position).magnitude)
                        {
                            closest = enemies[i].transform;
                        }
                    }
                    if ((closest.position - transform.position).magnitude < followDistance)
                    {
                        target = closest;
                    }
                }
            }
            else if ((target.position - transform.position).magnitude < followDistance)
            {
    ...
    private void OnCollisionStay2D(Collision2D collision)
    {
        if (!knockback)
        {
            if (captured && collision.gameObject.CompareTag("Enemy") && collision.gameObject.GetComponent<Enemy>().target == transform && !collision.gameObject.GetComponent<Enemy>().knockback)
            {
                health--;
                if (health <= 0)
                {
            ...
            else if (!captured && collision.gameObject.CompareTag("Ally") && !collision.gameObject.GetComponent<Enemy>().knockback)
            {
