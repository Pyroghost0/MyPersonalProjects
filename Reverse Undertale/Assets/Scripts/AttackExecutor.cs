...
public class AttackExecutor : MonoBehaviour
{
    ...
    IEnumerator TruckAttackCoroutine()
    {
        ...
        float velocity = Mathf.Pow(startMultiplier, .5f) * 3.1f;
        int num = 0;
        while (num < total && isAttacking)
        {
            ...
            do
            {
                ...
                yield return new WaitForSeconds(6f / velocity);
            } while (Random.value < .66f && num < total && isAttacking);
            yield return new WaitForSeconds(3.25f / velocity);
        }
        yield return new WaitForSeconds(.5f / velocity);
        enemyAnimator.SetTrigger("Attack End");
